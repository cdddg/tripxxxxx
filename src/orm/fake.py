...
from orm import member, supplier, tour, score
...
class MemberBase(factory.django.DjangoModelFactory):
    ...
    platform = Faker.random_element(
        [row.value for row in constants.PlatformType]
    )
    account = Faker.email()
    email = Faker.email()
    ...
    gender = Faker.random_element(
        [row.value for row in constants.GenderType]
    )
    birthday = Faker.date()
...
class TourGroupBase(factory.django.DjangoModelFactory):
    ...
    name = Faker.text()
    is_recommend = Faker.random_element(
        [row.value for row in constants.IsRecommendStatus]
    )
...
class ScoreBaseLog(factory.django.DjangoModelFactory):
    ...
    category = Faker.random_element(
        [row.value for row in constants.SupplierScoreCatagory]
    )
    food_score = Faker.pyint(1, 100)
    traffic_score = Faker.pyint(1, 100)
    scheduler_score = Faker.pyint(1, 100)
...
class TourGroupLocation(factory.django.DjangoModelFactory):
    ...
    type = Faker.random_element(
        [row.value for row in constants.TourGroupLocationType]
    )
    option = Faker.random_element(
        [row.value for row in constants.TourGroupLocationOpition]
    )
...
class TourGroupTag(factory.django.DjangoModelFactory):
    ...
    tour_group = factory.SubFactory(TourGroupBase)
    tag = Faker.random_element(
        [row.value for row in constants.TourGroupTag]
    )
...
class TourGroupBucket(factory.django.DjangoModelFactory):
    ...
    currency = Faker.random_element(
        [row.value for row in constants.Currency]
    )
